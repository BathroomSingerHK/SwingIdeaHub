...
str_time = now_hkt.strftime("%Y-%m-%d %H:%M:%S (HKT)")

today = datetime.now().date()
us_cal = USFederalHolidayCalendar()

# --- [CONFIGURATION] Day Settings ---
N_DAYS_DEFAULT = 20  # Default for most tickers
N_DAYS_GC = 20       # Specific setting for Gold (GC=F)
...
def load_hsi_from_excel(contract_code, excel_path="data/hsi_data.xlsx", n_days_to_load=20):
    """
    ...
    try:
        ...
        all_trading_dates = [d for d in all_dates]

        # 选择最近 N 个交易日
        if len(all_trading_dates) <= n_days_to_load:
            selected_trading_days = all_trading_dates
        else:
            selected_trading_days = all_trading_dates[-n_days_to_load:]
...
def process_ticker(ticker_symbol, start_date, end_date, n_days_to_load):
    ...
    if is_crypto:
        ...
        all_trading_dates = [d for d in all_dates if pd.Timestamp(d).weekday() < 5]
...
def plot_volume_profile(data_20d, name, n_days_label, gc_reference=None):
    """
    ...
    fig.update_layout(
        title=f'{name} – {n_days_label} Day Volume Profile<br>Spot Price ${current_price:.2f}',
        xaxis_title='Volume',
        yaxis_title='Price ($)',
        plot_bgcolor='black',
        paper_bgcolor='black',
...
def generate_dashboard(HSI_output=True):
    ticker_symbols = ["NQ=F","GC=F","SI=F", "ES=F", "NIY=F", "BTC-USD", "ETH-USD"]
    symbol_payloads = []
    ...
    for ticker_symbol in ticker_symbols:
            if ticker_symbol == gc_ticker:
                # [MODIFIED] Use N_DAYS_GC for Gold
                days_to_use = N_DAYS_GC
                result = process_ticker(ticker_symbol, start_date, end_date, days_to_use)
                if result is not None:
                    ...
                    # [MODIFIED] Pass days_to_use to plot
                    plot_result = plot_volume_profile(data_20d, name, days_to_use, gc_reference=None)
                    fig = plot_result['fig']
    ...
    for ticker_symbol in ticker_symbols:
        ...
        # [MODIFIED] Use N_DAYS_DEFAULT for others
        days_to_use = N_DAYS_DEFAULT
        result = process_ticker(ticker_symbol, start_date, end_date, days_to_use)
        if result is None:
            continue
        ...
        # [MODIFIED] Pass days_to_use to plot
        plot_result = plot_volume_profile(data_20d, name, days_to_use, gc_reference=gc_reference)
        fig = plot_result['fig']
        spot_price = plot_result['spot_price']
        profile_records = [
            {
    ...
    if HSI_output:
        hsi_contracts = ['HSI2511', 'HSI2512']
        # [MODIFIED] Use N_DAYS_DEFAULT for HSI as well
        days_to_use = N_DAYS_DEFAULT
        for contract_code in hsi_contracts:
            result = load_hsi_from_excel(contract_code, excel_path="data/hsi_data.xlsx", n_days_to_load=days_to_use)
            if result is None:
                continue
    ...
    <body>
        <main>
            <h1 style="text-align: center; margin: 0 0 5px 0;">Volume Profile Dashboard</h1>
            <p style="text-align: center; color: #888; font-size: 0.9em; margin-bottom: 20px;">
                Last Updated: {str_time}
            </p>
    ...
    output_filename = f'volume_profile_dashboard_{file_timestamp}.html'

    # Save to the new filename
    with open(output_filename, 'w', encoding='utf-8') as f:
        f.write(html_content)

    print(f"\nGenerated HTML file: {output_filename}")
    return {
        'html_path': output_filename,  # Return dynamic path for Telegram bot to find it
